...
def show():
    ...
    # ---------------- HIDE ONLY MOVED ENTRIES FROM DASHBOARD TABLE ----------------
    ...
    with col_h1:
        ...
        st.markdown(f"**🗳️ Voter Workspace**<br><span style='font-size:0.85rem; color:#475569;'>🕒 {current_dt}</span>", unsafe_allow_html=True)
    ...
    with c_b1:
        if st.button(f"📄 Total ({total_count})", use_container_width=True, key="btn_total"):
            st.session_state.status_filter = "All"
    with c_b2:
        if st.button(f"✅ Comp ({completed_count})", use_container_width=True, key="btn_completed"):
            st.session_state.status_filter = "Complete"
    with c_b3:
        if st.button(f"⏳ Pend ({pending_count})", use_container_width=True, key="btn_pending"):
    ...
    with d1:
        if st.button("🔄 All Time", use_container_width=True, key="btn_all_time"):
            st.session_state.day_filter = "All"
    with d2:
        if st.button("⏳ 7+ Days", use_container_width=True, key="btn_7_plus"):
            st.session_state.day_filter = "7_plus"
    with d3:
        if st.button("⚡ < 7 Days", use_container_width=True, key="btn_less_7"):
    ...
    if not df.empty:
        ...
        with col_btn1:
            btn_app = st.button("✅ Approve", use_container_width=True, type="primary", key="approve_btn")
        with col_btn2:
            btn_rej = st.button("❌ Reject", use_container_width=True, key="reject_btn")
        with col_btn3:
            btn_mov = st.button("📦 Move", use_container_width=True, key="move_btn")

        # Delete Button row next or separate
        col_del_btn, _ = st.columns([1, 2])
        with col_del_btn:
            btn_del = st.button("🗑️ Delete Selected", use_container_width=True, key="delete_records_btn")
